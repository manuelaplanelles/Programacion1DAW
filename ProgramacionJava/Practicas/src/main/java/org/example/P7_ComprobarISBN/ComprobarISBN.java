...
public class ComprobarISBN {
    ...
    public static int calcularSuma(String isbn) {
        int suma = 0;
    ...
    public static void repararISBN(String isbn) {
        ...
        for (int intento = 0; intento <= 9; intento++) {
            ...
            if (calcularSuma(isbnReparado) % 11 == 0) {
                System.out.println("El dígito que falta es " + intento);
                return;
            }
        }

        String isbnReparadoX = isbn.substring(0, posicion) + "X" + isbn.substring(posicion + 1);
        if (calcularSuma(isbnReparadoX) % 11 == 0) {
